...
export function useUseCasesByCategory(villageId: string | undefined, category: string) {
  return useQuery({
    queryKey: ['use-cases', villageId, category],
    queryFn: () => useCaseRepository.getByCategory(villageId as string, category),
...
export function useCreateUseCase() {
  return useMutation({
    mutationFn: (formData: FormData) => useCaseRepository.createUseCase(formData),
  });
}

export function useUpdateUseCase() {
  return useMutation({
    mutationFn: ({ id, data }: { id: string; data: any }) => useCaseRepository.updateUseCase(id, data),
  });
}
