...
import { SubmitDataDialog } from "@/components/SubmitDataDialog";
...
export default function NonIoTDetail() {
  ...
  if (isLoading || isLoadingEnt) {
    return (
      <div className="p-4 text-foreground font-display">Loading details...</div>
    );
  }
  ...
  return (
    <div className="flex-1 overflow-y-auto">
      ...
      <div className="grid-cell">
        <p className="kpi-label mb-3">Data Entry</p>
        <button
          onClick={() => setIsDialogOpen(true)}
          className="w-full bg-foreground text-background font-display text-xs font-medium py-2.5 hover:bg-foreground/90 transition-colors tracking-wider"
        >
          SUBMIT DATA
        </button>
        {!entityId && (
          <p className="text-[10px] text-muted-foreground font-body mt-2">
            No entity found yet for this use case — submission will be enabled
            once an entity is available.
          </p>
        )}
      </div>
                ...
                {entities.map((ent: any) => (
                  ...
                  >
                    ...
                    <td className="py-1.5 text-right">
                      {parseFloat(ent.latitude).toFixed(4)}
                    </td>
                    <td className="py-1.5 text-right">
                      {parseFloat(ent.longitude).toFixed(4)}
                    </td>
                  </tr>
                ))}
              </tbody>
